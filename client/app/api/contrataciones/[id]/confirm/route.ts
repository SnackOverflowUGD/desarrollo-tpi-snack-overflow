/**
 * Confirm Route Handler (BFF) — POST /api/contrataciones/[id]/confirm
 * (ADR-09-01, REQ-01). Clone of reject/route.ts: the browser POSTs to this
 * SAME-ORIGIN path; the handler runs server-side and uses `backendFetch` to
 * attach the session Bearer and forward to `POST /contrataciones/:id/confirm`
 * (NO body — identity is derived from the token, REQ-10). The token NEVER
 * reaches the client.
 *
...
export async function POST(
  ...
  let result;
  try {
    result = await backendFetch(`/contrataciones/${id}/confirm`, {
      method: "POST",
    });
  } catch {
